const team = [
  {
    id: "0",
    name: "Pankaj Singh",
    role: "Founder & CEO",
    img: require("./Assets/Images/pankaj.jpeg"),
    bio: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quibusdam magnam tempore quidem consequuntur ea rerum blanditiis ullam veritatis laborum quaerat?",
  },
  {
    id: "1",
    name: "Ritesh Gupta",
    role: "Frontend Developer",
    img: require("./Assets/Images/ritesh.JPG"),
  },
  {
    id: "2",
    name: "Chirag Yadav",
    role: "Backend Developer",
    img: require("./Assets/Images/placeholder.png"),
  },
];

const courses = [
  {
    id: "2",
    term: "2nd",
    courses: [
      {
        id: "LB201",
        courseName: "Law of Evidence (LB-201)",
        instructor: "Aakanksha Derashree",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/law-of-evidence.jpg"),
        contents: [
          {
            topicName: "GENERAL ISSUES RELATING TO LAW OF EVIDENCE",
            subTopics: [
              {
            name: "GENERAL ISSUES RELATING TO LAW OF EVIDENCE(Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Relevant Facts, Difference b/w Facts in Issue and Relevant Facts, Types of Evidence, Facts - Proved,Disproved and Not Proved [Free Preview]",
            subTopics: [
              {
                name: "Relevant Facts, Difference b/w Facts in Issue and Relevant Facts, Types of Evidence, Facts - Proved,Disproved and Not Proved (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Doctrine of relevancy, Difference b/w Admissibility and Relevance, Res gestate",
            subTopics: [
              {
                name: "Doctrine of relevancy, Difference b/w Admissibility and Relevance, Res gestate (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Section 7 and 8",
            subTopics: [
              {
                name: "Section 7 and 8(Paid)",
                videoLink: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Section 9 and 11",
            subTopics: [
              {
                name: "Section 9 and 11(Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Section 10, Object, Scope, Ingredients of the Section 10, PYQs [Free Preview]",
            subTopics: [
              {
                name: "Section 10, Object, Scope, Ingredients of the Section 10, PYQs (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
           {
            topicName: "Section 17, Sections 18, 19&20, Section 21, Section 25, Section 26, Section 27",
            subTopics: [
              {
                name: "Section 17, Sections 18, 19&20, Section 21, Section 25, Section 26, Section 27",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Section 30, Section 113, Section 114(b), Section 32",
            subTopics: [
              {
                name: "Section 30, Section 113, Section 114(b), Section 32 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Section 112 AND Validity of Expert,Hostile,Dumb and Child witness",
            subTopics: [
              {
                name: "Section 112 AND Validity of Expert, Hostile, Dumb and Child witness (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Law of Estoppel, Privileged communication, Professional, communications",
            subTopics: [
              {
                name: "Law of Estoppel, Privileged communication, Professional, communications (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB202",
        courseName: "Family Law 2 (LB-202)",
        instructor: "Beauty Gupta and Imama",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/family-law.jpg"),
        contents: [
          { 
            aboutTheAuthor: "Beauty and Imama are from National Law University, Delhi where they have excelled academically at their university. Beauty is part of philanthropic work called IDIA where she helps bring diversity in legal education. Imama is an UN Millenium fellow where her mission is to reduce poverty worldwide.",
            topicName: "Joint Hindu Family and Hindu Coparcenary",
            subTopics: [
              {
                name: "Final Joint Hindu Family (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Coparcenary(Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
              {
                name: "Mitak & Daya plus Karta (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
               {
                name: "Karta Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
               {
                name: "Property (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
               {
                name: "Important PYQ (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
            ],
          },
          {
            topicName: "Alienation",
            subTopics: [
              {
                name: "Alienation (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "RIGHTS AND DUTIES OF COPARCENERS AND ALIENEE (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "QUESTIONS ON ALIENATION.mp4(Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
            ],
          },
          {
            topicName: "Partition",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
              {
                name: "Part 3 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 4 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName:
              "General Introduction and the Application of the Hindu Succession Act, 1956",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "QUESTION ON GENERAL INTRODUCTION AND APPLICATION OF HSA (Paid)",
                videoLink: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
            ],
          },
          {
            topicName: "Succession to the Property of Male Intestate",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoLink: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Succession to the Property of Female Intestate(Free Preview)",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Succession to the Property of Female Intestate(Free Preview)",
            subTopics: [
              {
                name: "Part 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
               {
                name: "Questions on Female Interstate Succession.mp4 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
               {
                name: "CASE SUMMARIES FOR SUCCESSION TO THE PROPERTY OF FEMALE INTERSATE(Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
            ],
          },
          {
            topicName: "Hindu Women's Estate",
            subTopics: [
              {
                name: "Hindu Women's Estate (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "LAWS OF GIFT UNDER MUSLIM LAW",
            subTopics: [
              {
                name: "HIBA Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "HIBA Part2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "CASE SUMMARIES-GIFT UNDER MUSLIM LAW (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
          {
            topicName: "Inheritance Under Muslim Law",
            subTopics: [
              {
                name: "Inheritance under Muslim Law (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
              {
                name: "Part 2 and Questions (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "6th July 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB203",
        courseName: "Law of Crimes - II (LB-203)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/lawofcrimes.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB204",
        courseName: "Property Law (LB-204)",
        instructorName: "Stuti Rastogi",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/propertylaw.jpg"),
        contents: [
          {
            aboutTheAuthor: "Instructor’s Bio- She has completed her LLB from Campus Law Centre, Delhi University and LLM from Faculty of Law, Delhi University. She has the teaching experience of more than one year in the field of law.",
            topicName: "Movable/Immovable Property (Section 3) [Free Preview]",
            subTopics: [
              {
                name: "Movable/Immovable Property (Section 3) [Free]",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Attestation (Section 3)",
            subTopics: [
              {
                name: "Attestation (Sec. 3) (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Notice (Section 3)",
            subTopics: [
              {
                name: "Notice (Sec. 3) (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Meaning of Transfer of Property (Sec. 5)",
            subTopics: [
              {
                name: "Meaning of Transfer of Property (Sec. 5) (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "What Kind of Property can be transferred [Sec. 6(a) and 43]",
            subTopics: [
              {
                name: "What Kind of Property can be transferred [Sec. 6(a) and 43] (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Conditional Transfer (Sec. 10, 11 and 40) [Free Preview]",
            subTopics: [
              {
                name: "Conditional Transfer (Sec. 10, 11 and 40) (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Transfer for the benefit of unborn persons (Sec. 13-18)",
            subTopics: [
              {
                name: "To be uploaded on: 8th July 2024",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Vested and Contingent interests (Sec. 19 and 21)",
            subTopics: [
              {
                name: "To be uploaded on: 8th July 2024",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Transfer during pendency of litigation (Sec. 52)",
            subTopics: [
              {
                name: "Transfer during pendency of litigation (Sec. 52) (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Mortgage (Sec. 58-60, 100)",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Lease and License (Sec. 105, 106 and Indian Easement Act, 1882 Sec. 4 & 52)",
            subTopics: [
              {
                name: "To be uploaded on: 8th July 2024",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Gift (Sec. 122-126)",
            subTopics: [
              {
                name: "Gift (Sec. 122-126) (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB205",
        courseName: "Public International Law (LB-205)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/publicintlaw.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
    ],
  },
  {
    id: "4",
    term: "4th",
    courses: [
      {
        id: "LB401",
        courseName: "Constitutional Law - II (LB-401)",
        instructor: "Avlin Singh",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/constitutionallaw.jpg"),
        contents: [
          {
            aboutTheAuthor: "I'm Avlin Singh. I've been serving as an Assistant Professor at Chandigarh University for the past two years, nurturing young minds while pursuing my Ph.D. in IPR from Rajiv Gandhi National University of Law, Patiala. My legal journey began at Khalsa College of Law, Amritsar, where I graduated in 2018 under Guru Nanak Dev University's guidance. Continuing my academic pursuit, I completed my LL.M. from the same university in 2021. My academic focus lies in criminal, and constitutional law, areas that I find particularly intriguing and enriching. Beyond the academic realm, I find joy in the arts—drawing, dancing, and losing myself in the melodies of music. My diverse interests shape my multifaceted personality, blending intellect with creativity. ",
            topicName: "State' under article 12 [Free Preview]",
            subTopics: [
              {
                name: "Article 12 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Law Under Article 13",
            subTopics: [
              {
                name: "Article 13 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Personal Laws",
            subTopics: [
              {
                name: "Triple Talak (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "3rd July 2024",
              },
            ],
          },
          {
            topicName: "Equality before the law: Article 14",
            subTopics: [
              {
                name: "Article 14 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Right to Equality",
            subTopics: [
              {
                name: "Article 15 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Right to Equality of Opportunity-Article 16",
            subTopics: [
              {
                name: "Article 16 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Right to Freedom- Article 17 & 18",
            subTopics: [
              {
                name: "Article 17 & 18 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Freedom of Speech and Expression: Article 19",
            subTopics: [
              {
                name: "Article 19 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Protection in respect of conviction for offenses: Article 20",
            subTopics: [
              {
                name: "Article 20 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Right to Education: Article 21A",
            subTopics: [
              {
                name: "Article 21A (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Protection against arrest and detention: Article 22",
            subTopics: [
              {
                name: "Article 22 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Right to Freedom of Religion: Article 25 & 26 [Free Preview]",
            subTopics: [
              {
                name: "Article 25 & 26 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "7th July 2024",
              },
            ],
          },
          {
            topicName: "Educational and Cultural Rights",
            subTopics: [
              {
                name: "Article 29 & 30 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Right to Constitutional Remedies (Article 32)",
            subTopics: [
              {
                name: "Article 32 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Fundamental Duties [Free Preview]",
            subTopics: [
              {
                name: "Fundamental Duties (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
          {
            topicName: "Amendment of Constitution",
            subTopics: [
              {
                name: "To be uploaded on: 10th July 2024",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "30th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB402",
        courseName: "Administrative Law (LB-402)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/administrativelaw.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB403",
        courseName: "Labour Law (LB-403)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/labourlaw.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB4031",
        courseName: "Gender Justice & Feminist Jurisprudence (LB-4031)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/womenrights.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB4033",
        courseName: "Competition Law | 2023 (LB-4033)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/competitionlaw.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB4036",
        courseName: "Intellectual Property Rights Law - I (LB-4036)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/intellectual.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
    ],
  },
  {
    id: "6",
    term: "6th",
    courses: [
      {
        id: "LB601",
        courseName:
          "Advocacy Professional Ethics and Accountancy for Lawyers (LB-601)",
        instructor: "Garima Garg",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/advocacy.jpg"),
        contents: [
          {
            aboutTheAuthor: "Hello, future legal legends! I'm Garima , your enthusiastic guide through the wild world of law, where the gavel meets the giggles. With a passion for justice and a knack for making the mundane magnificent, I promise to make our legal journey as entertaining as it is educational.Let's kick things off with my background. I earned my LLB Honors from the GGSIPU. I then polished my legal prowess with an LLM in Constitutional Law where I scored an impressive 80%—because why not aim for an A+ in awesomeness? My research and conference presentations have been so riveting, they've even won best paper awards. Who knew the law could be so award-winningly fun?My career is as diverse as a legal textbook. I've been a Junior Legal Associate, an NGO Coordinator, and recently, your favorite constitutional law educator at Unacademy. From juggling legal briefs to leading online classes, I bring a mix of real-world experience and a love for turning legalese into plain, hilarious English.In my classes, prepare for a rollercoaster ride through the legal landscape. Expect rigorous academic content delivered with a side of humor, real-world case studies spiced with sarcasm, and interactive discussions that make even the driest statutes sparkle. We'll dissect cases with surgical precision and laugh at the quirks of the legal system. Who says law can't be fun?When I'm not turning law into a comedy show, you'll find me strumming the sitar, crafting quirky art, or traveling to places where I can gather more material for my next legal pun. My passion for social justice and education keeps me motivated to inspire you to think critically, argue convincingly, and laugh heartily.Join me in this legal extravaganza where we'll make learning law an adventure filled with laughter, insight, and maybe a few courtroom puns. Let's decode the complexities of law with a smile and prove that justice doesn't have to be boring. Get ready for a law-some time!",
            topicName: "The Advocates Act,1961-Introduction [Free Preview]",
            subTopics: [
              {
                name: "The Advocates Act,1961-Introduction (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Bar Councils",
            subTopics: [
              {
                name: "Bar Councils (Paid)",
                videoLink: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Admissions and Enrollment of Advocates",
            subTopics: [
              {
                name: "Admissions and Enrollment of Advocates (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Indira Jaising (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Right to Practice [Free Preview]",
            subTopics: [
              {
                name: "Right to Practice (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Conduct of Advocates and Disciplinary Proceedings",
            subTopics: [
              {
                name: "Conduct Of Advocates and Disciplinary Proceedings (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Contempt of Courts Act, 1971",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "DEFENCES AVAILABLE IN CONTEMPT OF COURT",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "CONTEMPT BY JUDGES & MAGISTRATES [Free Preview]",
            subTopics: [
              {
                name: "Contempt By Judge & Magistrates (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Punishment for Contempt",
            subTopics: [
              {
                name: "Punishment for Contempt (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Procedure Of Contempt",
            subTopics: [
              {
                name: "Procedure Of Contempt (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Rules Governing Advocates [Free Preview]",
            subTopics: [
              {
                name: "Part 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Part 3 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Summary of Case laws",
            subTopics: [
              {
                name: "Summary Of Case Laws 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Summary Of Case Laws 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Summary Of Case Laws 3 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB602",
        courseName: "Alternative Dispute Resolution (LB-602)",
        instructor: "Sonali Gambhir",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/Alternative-Dispute-Resolution.jpg"),
        contents: [
          {
            aboutTheAuthor: "Hello students! I am Sonali and with over 3 years of Post-Qualification Experience (PQE) in Litigation and Dispute Resolution, I excel in both collaborative efforts and handling independent clients. I have completed my LLB from Faculty of law, University of Delhi. Afterwards I have done post graduate diploma in ADR from Indian Law Institute (ILI), New Delhi.I am a seasoned Advocate practicing in the courts of Delhi, with extensive experience appearing before the Hon'ble Supreme Court of India, Delhi High Court, Quasi-Judicial Authorities, and District Courts of Delhi.My strong academic background and profound expertise in procedural laws complement my practical experience. I am passionate about teaching and I hope that you will enjoy my lectures.I can be reached at sonali.gambhir96@gmail.com. Thank you! ",
            topicName: "INTRODUCTION TO ALTERNATE DISPUTE RESOLUTION [Free Preview]",
            subTopics: [
              {
                name: "Introduction (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "NEGOTIATION: INTRODUCTION, STYLES AND STATERGIES [Free Preview]",
            subTopics: [
              {
                name: "NEGOTIATION: INTRODUCTION, STYLES AND STATERGIES (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Mediation/Conciliation",
            subTopics: [
              {
                name: "Mediation/Conciliation (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Arbitration",
            subTopics: [
              {
                name: "Arbitration (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB603",
        courseName: "Environmental Law (LB-603)",
        instructor: "Sameera Fatima",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/environmental_law.jpg"),
        contents: [
          {
            aboutTheAuthor: "Sameera Fatima is an accomplished academic with a foundation in law, holding a BBA LLB and an LLM. Currently, She is pursuing a Ph.D. at the National University of Study and Research in Law (NUSRL), Jharkhand.She is an Assistant Professor in Law, with a rich experience in the realm of academics, research and paper publications.",
            topicName: "INTERNATIONAL ENVIRONMENTAL LAW [Free Preview]",
            subTopics: [
              {
                name: "Part 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 3 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "FUNDAMENTAL PRINCIPLES OF ENVIRONMENTAL PROTECTION",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 3 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "CONSTITUTIONAL PERSPECTIVE",
            subTopics: [
              {
                name: "Constitutional Perspective (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "PREVENTION AND CONTROL OF WATER POLLUTION",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "ENVIRONMENTAL PROTECTION",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "NATIONAL GREEN TRIBUNAL [Free Preview]",
            subTopics: [
              {
                name: "Part 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "PROTECTION AND CONSERVATION OF FORESTS, BIODIVERSITY AND WILDLIFE",
            subTopics: [
              {
                name: "Part 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Part 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB604",
        courseName: "Principles of Taxation Law (LB-604)",
        instructor: "Shaweta Gupta",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/property.jpg"),
        contents: [
          {
            aboutTheAuthor: "I am a qualified company secretary and semi qualified Chartered Accountant and having a great knowledge in the field of commerce all subjects like:- economics, accountancy, cost accounting, management accounting, business law, corporate law, labour law, taxation and all other subjects too. As I am in the education line from the past 9 years and work as an Online educator on different learning programs.",
            topicName: "Introduction",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Lecture 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Agriculture Income",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Lecture 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Residence and Scope of Total Income",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Lecture 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
              {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Heads of Income- Salary [Free Preview]",
            subTopics: [
              {
                name: "Lecture 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Heads of Income: House property",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Heads of Income: profit and gains from business or Professions",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Lecture 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Heads of Income- Capital Gains",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Heads of Income: other sources",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Income of Other Persons included in Assessee’s Total Income",
            subTopics: [
              {
                name: "Lecture 1 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Lecture 2 (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
               {
                name: "Notes (Paid)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
           {
            topicName: "Assessment [Free Preview]",
            subTopics: [
              {
                name: "Lecture 1 (Free)",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
      {
        id: "LB6032",
        courseName: "Insurance and Banking Law (LB-6032)",
        instructor: "Ada Wallace",
        instructorID: "0",
        price: "299.99",
        img: require("./Assets/Images/banking.jpg"),
        contents: [
          {
            topicName: "Topic 1",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 2",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
          {
            topicName: "Topic 3",
            subTopics: [
              {
                name: "Sub Topic 1",
                videoUrl: "#",
                textLink: "#",
                pyqs: [
                  {
                    year: "2023",
                    marks: 10,
                  },
                  {
                    year: "2021",
                    marks: 20,
                  },
                ],
                lastUpdated: "15th June 2024",
              },
            ],
          },
        ],
      },
    ],
  },
];

export { team, courses };
